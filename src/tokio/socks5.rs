...
impl AsyncProxy for SOCKS5Proxy {
    async fn connect(&self, addr: impl ToTarget + Send) -> Result<TcpStream> {
        ...
        if auth_method == 0x02 {
            self.tokio_authenticate(&mut stream).await?;
        }
...
impl SOCKS5Proxy {
    async fn tokio_connect(&self, stream: &mut TcpStream) -> Result<u8> {
        ...
        if buffer[1] == 0xFF { //NO ACCEPTABLE METHODS
            return Err(Box::new(Socks5HandshakeError {
                socket: peer,
            }));
        }
    ...
    async fn tokio_authenticate(&self, stream: &mut TcpStream) -> Result<()> {
        let peer = stream.peer_addr()?;

        let request = match &self.auth {
            Some(auth) => make_socks5_authentication_request(&auth),
            None => return Err(Box::new(Socks5HandshakeError {
                socket: peer,
            })),
        };
        stream.write_all(&request).await?;
    ...
    async fn tokio_request(&self, stream: &mut TcpStream, addr: impl ToTarget) -> Result<()> {
        ...
        if buffer[1] != 0x00 { //Request refused
            return Err(Box::new(Socks5RequestError {
                socket: peer,
                error_type: buffer[1],
            }));
        }
